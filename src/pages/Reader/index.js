...
export default class ReaderPage extends React.Component {
    ...
    fetchContent = (index) => {
        ...
        let lineWidth = Math.floor((width - this.state.fontSize) * 2 / this.state.fontSize);
        let lines = parseContent(article, lineWidth);
        lines.unshift("\n");
        // lines.unshift(article);
        console.log(lineWidth,width/this.state.fontSize,height/this.state.fontSize);
        console.log(lines);
        /*this.setState({
         next: false,
         page: 1,
        ...
        return <View><Text style={{fontSize:24,lineHeight:24}}>{lines}</Text></View>;
    };
